...
update_config()
{
    ...
    if [ $deployment_method == "local" ]; then
        ...
        sudo sed -i "s/lexical_resources_API_host_name = .*/lexical_resources_API_host_name = '$lexical_resources_API_host_name'/" app/requestLexicalResources.py
        sudo sed -i "s/lexical_resources_API_port = .*/lexical_resources_API_port = '$lexical_resources_API_port'/" app/requestLexicalResources.py
        echo ""
    fi
